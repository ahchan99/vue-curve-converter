...
import { solveEllipsePoints } from './algorithm/ellipse'
import { solveHelixPoints } from './algorithm/helix'
...
export class ObjectFactory {
    ...
    create(data) {
        ...
        if (data.value === 'helix')
            return this.drawHelix(data.a, data.b, data.x, data.y, data.turnRound, data.begin, data.end)
    }
    ...
    solve(data, factor) {
        ...
        if (data.value === 'ellipse')
            //得到解算出来的点
            this.outputPoints = solveEllipsePoints(data, factor)
        if (data.value === 'helix')
            //得到解算出来的点
            this.outputPoints = solveHelixPoints(data, factor)
    ...
    getSimulationMovePoints() {
        ...
        console.log('curvePath.getLength' + curvePath.getLength())
        let output = curvePath.getSpacedPoints(parseInt(curvePath.getLength()) * 10)
        return output
    }
    ...
    deserializationCurve(data) {
        ...
        data.beginPoint = parseFloat(data.beginPoint)
        data.baseRadius = parseFloat(data.baseRadius)
    ...
    drawEllipse(a, b, x, y, r) {
        ...
        let curve = new THREE.EllipseCurve(
            ...
            false,            // aClockwise
            Math.PI * r * 2 / 360                 // aRotation
        );

        let points = curve.getPoints((a + b) * 100);
    ...
    //画螺旋线
    drawHelix(a, b, x, y, r, t1, t2) {

        //存放点的数组
        let points = []
        let range = (t2 - t1) / 180 * Math.PI
        let count = range / Math.PI * 100
        for (let i = 0; i < count; i++) {

            let t = range * i / count + t1 / 180 * Math.PI
            points.push(new THREE.Vector2((a + b * t) * Math.cos(t),
                (a + b * t) * Math.sin(t)))
        ...
        helix.setRotationFromEuler(new THREE.Euler(0, 0, r / 180 * Math.PI, "XYZ"));
        helix.position.set(x,y,0)
