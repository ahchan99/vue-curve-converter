 ...
 */
export const solveHelixPoints = function (curve, solve) {
    ...
    const getX = function (p) {
        ...
        return (curve.a + curve.b * p) * Math.cos(p)

    }

    const getY = function (p) {

        return (curve.a + curve.b * p) * Math.sin(p)

    }

    const getSlope = function (p) {

        return (curve.b * Math.sin(p) + (curve.a + curve.b * p) * Math.cos(p)) / (curve.b * Math.cos(p) - (curve.a + curve.b * p) * Math.sin(p));

    }
    const getSlope2 = function (p) {

        let dx = curve.b * Math.cos(p) - (curve.a + curve.b * p) * Math.sin(p)
        let dy = curve.b * Math.sin(p) + (curve.a + curve.b * p) * Math.cos(p)
        let dx2 = Math.cos(p) - p * Math.sin(p) - dy
        let dy2 = Math.sin(p) + curve.b * Math.cos(p) + dx

        return (dy2 * dx - dy * dx2) / Math.pow(dx, 3)
    ...
    //返回-PI到PI
    let min = 0
    let range = (curve.end - curve.begin) / 180 * Math.PI
    let max = min + range
    let degree = Math.PI * curve.turnRound / 180
    let msg = ''

    if ((max - min) > 2 * Math.PI && Math.abs(curve.b * min - curve.b * (min + 2 * Math.PI)) < solve.D)
        alert('警告:整个零件发生严重过切\n')
